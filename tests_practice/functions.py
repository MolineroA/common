...
def quantity_a_in_str(my_str: str):
    """
...
def add_until_a_single_digit(num: int):
    """
...
def number_not_occur_twice(in_list):
    ...
    Output : 5
    """
    return list([i for i in in_list if in_list.count(i) < 2])[0]
...
def convert_number_to_hours(num: int):
    """
...
def largest_word(in_string: str):
    """
...
def fibonacci(num: int):
    ...
    fib_num = fib()
    return [next(fib_num) for _ in range(num)]
...
def replace_letters_in_str(my_str: str):
    """
    Write a program that will take the str parameter being passed
